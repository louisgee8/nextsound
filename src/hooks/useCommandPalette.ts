...
export const useCommandPalette = ({ onItemSelect, onClose }: UseCommandPaletteProps) => {
  ...
  const [mockSearchResults, setMockSearchResults] = useState<
    Array<{ track: ITrack; isExactMatch: boolean }>
  >([])
      ...
      const searchTimeout = setTimeout(() => {
        try {
          ...
          setMockSearchResults(
            results.map((result) => ({ track: result.track, isExactMatch: result.isExactMatch })),
          )
        } catch (error) {
  ...
  const filteredCommands = useMemo(() => {
    ...
    return commands
      ...
      .map((command) => {
        ...
        const isExactMatch =
          command.title.toLowerCase() === searchTerm ||
  ...
  const handleItemSelect = (item: SearchResult) => {
    ...
    if (query.trim()) {
      const newSearches = [query.trim(), ...recentSearches.filter((s) => s !== query.trim())].slice(
        0,
        10,
      )
