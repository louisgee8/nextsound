...
import { SPOTIFY_API_BASE_URL, SPOTIFY_CLIENT_ID, USE_BACKEND_PROXY } from '@/utils/config'
  ...
  responseHandler: async (response) => {
    if (!response.ok) {
      ...
      throw new APIError({
        ...
        code:
          (typeof errorData === 'object' && errorData?.error?.status) || `HTTP_${response.status}`,
...
const baseQueryWithRetry = async (args: any, api: any, extraOptions: any) => {
  ...
  for (let attempt = 1; attempt <= RETRY_CONFIG.maxRetries + 1; attempt++) {
    try {
      ...
      console.log(
        `Retrying request (attempt ${attempt}/${RETRY_CONFIG.maxRetries}) after ${retryDelay}ms delay`,
      )
...
export const spotifyApi = createApi({
  ...
  endpoints: (builder) => ({
    ...
    >({
      // limit default matches Spotify's current /search cap (10 as of 2026-04-26;
      // was 50 pre-2024, briefly 20 in late 2024). Passing higher = silent 400.
      query: ({ query, type, limit = 10, offset = 0 }) =>
...
export const { useSearchMusicQuery, useGetTrackQuery, useGetAlbumQuery, useGetArtistQuery } =
  spotifyApi
